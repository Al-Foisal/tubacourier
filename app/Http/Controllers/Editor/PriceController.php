...
class PriceController extends Controller
{
    ...
    $name = hexdec(uniqid()) . '.' . $request->file('image')->getClientOriginalExtension();
    $image_file = 'product/image/'. $name;

    $request->file('image')->move('product/image/', $name);

   };

        $store_data = new Price();
        $store_data->image = $image_file;
    ...
    public function edit($id){
     ...
     public function update(Request $request){
        ...
        $update_data = Price::find($request->hidden_id);
            ...
            if(file_exists($update_data->image)){
