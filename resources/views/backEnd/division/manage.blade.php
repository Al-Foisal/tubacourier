                    ...
                    <div class="col-sm-12">
                      <div class="manage-button">
                        <div class="body-title">
                          <h5>Manage Division</h5>
                        </div>
                        <div class="quick-button">
                          <a href="{{url('admin/division/add')}}" class="btn btn-primary btn-actions btn-create">
                          ...
                          {{--  <td>{{$value->status==1? "Active":"Inactive"}}</td>  --}}
                           <td>
                            <ul class="action_buttons">
                                {{--  <li>
                                  @if($value->status==1)
                                  <form action="{{url('admin/division/inactive')}}" method="POST">
                                    @csrf
                                    <input type="hidden" name="hidden_id" value="{{$value->id}}">
                                    <button type="submit" class="thumbs_up" title="unpublished"><i class="fa fa-thumbs-up"></i></button>
                                  </form>
                                  @else
                                    <form action="{{url('admin/division/active')}}" method="POST">
                                      @csrf
                                      <input type="hidden" name="hidden_id" value="{{$value->id}}">
                                  ...
                                  <li>
                                      <a class="edit_icon" href="{{url('admin/division/edit/'.$value->id)}}" title="Edit"><i class="fa fa-edit"></i></a>
                                  </li>
                                  <li>
                                    <form action="{{url('admin/division/delete')}}" method="POST">
                                      @csrf
                                      <input type="hidden" name="hidden_id" value="{{$value->id}}">
