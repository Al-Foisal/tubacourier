...
@section('content')
    <div class="profile-edit mrt-30">
        <div class="row">
            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
                <form action="{{ url('editor/dliveryman-asign/bulk-option') }}" method="POST" id="myform"
                    class="">
                    @csrf
                    
                    <select class="form-control" name="parceltype" style="width: 20%;float:left;margin-right:2rem;">
                        <option value="">Select Status</option>
                        @foreach ($parceltypes as $item)
                            <option value="{{ $item->id }}">{{ $item->title }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="btn btn-primary" style="float: left;">Apply</button>
                ...
                <td>
                    ...
                    <!-- Modal end -->
                    <li><button class="btn btn-danger" title="Action" data-toggle="modal"
                            data-target="#sUpdateModal{{ $value->id }}"><i class="fa fa-sync-alt"></i></button></li>
                    <!-- Modal -->
                                            ...
                                            <select name="status" onchange="percelDelivery(this)" class="form-control"
                                                ...
                                                @foreach ($aparceltypes as $key => $ptvalue)
                                                    <option
                                                        ...
                                                        @if ($value->status > $ptvalue->id) disabled @endif>
                    ...
                    <!-- Modal end -->
                    @if ($value->status >= 2)
                        <li><a class="btn btn-primary" a href="{{ url('agent/parcel/invoice/' . $value->id) }}"
                                title="Invoice"><i class="fas fa-list"></i></a></li>
                    @endif
                </td>
                </tr>
                @endforeach
